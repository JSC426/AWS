...
def make_spark_config(master_type, slave_type, slave_number):
    """
    create proper spark driver/executor config based on instance cpu/memory specs
    """
    configs = {}
    configs['Classification'] = "spark-defaults"
    configs['Properties'] = {}
    ...
    executor_per_slave  = floor((core_per_slave_ec2-1) / core_per_executor)
    num_executors       = slave_number * executor_per_slave - 1
    mem_per_executor    = floor(mem_per_slave_ec2 / executor_per_slave * 0.90)
    overhead_executor   = floor(mem_per_slave_ec2 / executor_per_slave) - mem_per_executor
    configs['Properties']['spark.executor.memory'] = str(mem_per_executor) + "G"
    configs['Properties']['spark.executor.memoryOverhead'] = str(overhead_executor) + "G"
    configs['Properties']['spark.executor.cores'] = str(core_per_executor)
    configs['Properties']['spark.executor.instances'] = str(num_executors)

    # driver configs
    cores_driver        = df_ec2_specs[master_type]['core']
    mem_driver          = floor(df_ec2_specs[master_type]['mem_g'] * 0.80)
    overhead_mem_driver = df_ec2_specs[master_type]['mem_g'] - mem_driver
    configs['Properties']['spark.driver.memory'] = str(mem_driver) + "G"
    configs['Properties']['spark.driver.memoryOverhead'] = str(overhead_mem_driver) + "G"
    configs['Properties']['spark.driver.cores'] = str(cores_driver)
    configs['Properties']['spark.driver.maxResultSize'] = str(mem_driver) + "G"

    # misc configs
    configs['Properties']['spark.serializer'] = "org.apache.spark.serializer.KryoSerializer"

    configs['Properties']['spark.serializer.objectStreamReset'] = '-1'
    configs['Properties']['spark.kryoserializer.buffer.max'] = '2048m'
    configs['Properties']['spark.dynamicAllocation.enabled'] = 'true'

    configs['Properties']['spark.sql.tungsten.enabled'] = 'true'
    configs['Properties']['spark.sql.parquet.mergeSchema'] = 'false'
    configs['Properties']['spark.sql.execution.arrow.enabled'] = 'true'
    configs['Properties']['spark.sql.execution.arrow.pyspark.enabled'] = 'true'
    configs['Properties']['spark.sql.execution.arrow.pyspark.fallback.enabled'] = 'true'
    configs['Properties']['spark.rdd.compress'] = 'true'
    configs['Properties']['spark.executorEnv.ARROW_PRE_0_15_IPC_FORMAT'] = '1'
    configs['Properties']['spark.sql.pyspark.jvmStacktrace.enabled'] = 'true'
    configs['Properties']['spark.yarn.appMasterEnv.ARROW_PRE_0_15_IPC_FORMAT'] = '1'
    configs['Properties']['spark.hadoop.parquet.enable.summary-metadata'] = 'false'
    configs['Properties']['spark.scheduler.listenerbus.eventqueue.capacity'] = '1000000'

    print("Cores     : Master(%d) + %d x Executors (%d)"
        % (cores_driver, num_executors, core_per_executor))
    print("Memory (G): Master(%d + %d) + %d x Executors (%d + %d)" 
        % (mem_driver, overhead_mem_driver, num_executors, mem_per_executor, overhead_executor))
...
def make_emr_config(master_type, slave_type, slave_number):
    ...
    spark_config_dict = make_spark_config(master_type, slave_type, slave_number)
